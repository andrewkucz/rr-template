...
export function searchParamsToSearchRecord(
...
function searchRecordToSearchParams(search: SearchRecord): URLSearchParams {
...
export function createSearchParamsNavigate(
	setSearchParams: SetURLSearchParams,
): NavigateFn {
	return ({ search, replace }) => {
		setSearchParams(
			(prev) => {
				const prevRecord = searchParamsToSearchRecord(prev);
				const nextRecord =
					search === true
						? prevRecord
				...
				return searchRecordToSearchParams(nextRecord);
			},
			{ replace },
		);
	};
		...
		for (const cfg of columnFiltersCfg) {
			...
			if (cfg.type === "string") {
				const value = Array.isArray(raw)
					? ((deserialize(raw[0]) as string) ?? "")
					: ((deserialize(raw) as string) ?? "");
