...
app/
  ...
  routes.ts             # route config (react-router config-based routing)
...
**Request flow:** Browser → React Router route → loader/action (server) or component (client). Client data fetching goes through tRPC at `/api/trpc`, handled by `routes/api/trpc.ts`.
...
Routes are declared in `app/routes.ts` using `@react-router/dev/routes` helpers (`index`, `route`, `prefix`, etc. Docs: https://reactrouter.com/start/framework/routing). File-based routing is **not** used.
...
After every change, both linting and type checking must pass. Run the following command to check both:
```bash
npm run check
```
